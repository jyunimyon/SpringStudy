...
-----------------------
## section 6-회원 관리 예제_웹 MVC 개발
### 17강 회원 웹 기능- 홈 화면 추가
* Controller내에 homeController와 templates내에 home.html 구현
  * @GetMapping() --> 요청한 url이 ()의 url과 일치할 때 이동할 html을 반환
    ```Java
    //homeController 일부
    @GetMapping("/")
    public String home() {
  ...
  * 회원 가입과 목록의 url 지정
    ...
    ```
* **Spring Container안에 home 관련 컨트롤러가 있으므로 hello-static.html이 아닌 home.html을 반환할 수 있는 것**
  * 즉, spring contoller안에 요청 html이 존재하지 않을 경우 static 파일을 반환
--------------
### 18강 회원 웹 기능- 회원 가입
* MemberController 추가 작성 및 templates내에 members/createMemberForm.html 구현  
